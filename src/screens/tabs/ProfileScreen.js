...
export default class ProfileScreen extends Component {
  ...
  // send GET request to obtain student's details from the server
  getData = async () => {
    const token = await AsyncStorage.getItem("userToken");
    axios
      .get("http://157.245.205.223:8000/student", {
        headers: {
  ...
  changeAvatar = async () => {
    ...
    axios
      .post(
        "http://157.245.205.223:8000/profile_pic",
        {
        ...
        <ScrollView showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.userInfo}>
            ...
            <Text style={[styles.text, { fontSize: 18 }]}>
              House: {this.houseID_to_string(this.state.house)}
            </Text>
            <Text
              style={[
                styles.text,
                { color: "#AEB5BC", fontSize: 14, marginTop: 20 },
              ]}
            >
              Student
            </Text>
          ...
          <View style={styles.recycleStats}>
            <View
              style={[styles.totalContainer, { backgroundColor: "#DAA520" }]}
            >
              <Text style={styles.countItem}>
                {this.state.BlueRecycled +
                  this.state.BrownRecycled +
                  this.state.OrangeRecycled}
              </Text>
